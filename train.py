...
def train_model(model, optimizer, config, train_dataset, test_dataset):
    ...
    # all_pairs = torch.tensor([(attr2idx[attr], obj2idx[obj])
    #                       for attr, obj in train_dataset.pairs]).to(model.device)
    ...
    for i in range(config.epoch_start, config.epochs):
        ...
        for bid, batch in enumerate(train_dataloader):
            # if config.model_name == 'UOTCompPrototype':
            #     predict = model(batch, all_pairs)
            # else:
            predict = model(batch, train_pairs)
            loss = model.loss_calu(predict, batch, train_pairs)
...
if __name__ == "__main__":
    ...
    # val_dactaset = CompositionDataset(dataset_path,
    #                                  phase='val',
    #                                  split='compositional-split-natural')
