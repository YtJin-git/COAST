...
class Evaluator:
    ...
    def evaluate_predictions(
        ...
        area = np.trapezoid(seen_accuracy, unseen_accuracy) if hasattr(np, 'trapezoid') else np.trapz(seen_accuracy,
                                                                                                      unseen_accuracy)
...
def predict_logits_text_first(model, dataset, config):
    ...
    with torch.no_grad():
        ...
        for i_text_batch in range(num_text_batch):
            cur_pair = pairs[
                i_text_batch * config.text_encoder_batch_size:(i_text_batch + 1) * config.text_encoder_batch_size, :]
        ...
        for i_item in range(len(text_feats)):
            ...
            # text_feats[i_item] = F.normalize(text_feats[i_item], p=2, dim=-1)
...
def test(
    ...
    results = evaluator.score_model(
        all_pred_dict, all_obj_gt, bias=1e3, topk=config.topk, config=config
    )
...
if __name__ == "__main__":
    ...
    model = get_model(config, attributes=attributes, classes=classes, offset=offset, dset=val_dataset).to(device)
